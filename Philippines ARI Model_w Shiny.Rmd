...
11. The population at risk at the beginning of the year (before replacements) is the sum of all infected and uninfected in the previous year. Therefore, the population at risk should be the population from the previous year excluding those with disease. 
12. New infections are generated by applying the ARI to uninfected and current infections (1-5 year and 6+ year infections). The disease treatment cures from the previous year are added back into the model as new infections in the current year. 
13. Infections of 1-5 year durations are generated by advancing those infected without a reinfection or disease progression to the next year (e.g. those with a 3-year infection in previous year who were not newly infected or progressed to disease move on to become a 4-year infection).  
14. Old infections are a sum of old infections in previous year and those who advanced from a 5 year infection. 
15. The infected deaths from the previous year are removed from the current year’s infection groups. This removal is proportional to group size (i.e. a much higher proportion of deaths are removed from the old infections compared to new or 1-5 infections). 
16. Those with disease who self-cured in the previous year (6.62% of prevalent disease) are added back into model as new infections. 
17. Infected and uninfected deaths are calculated as per steps 8 and 9 above. 
18. Disease progression is computed per probability of progression to disease outlined in static parameters. 
19. Group of everyone with prevalent disease is the sum of incident disease and prevalent disease from previous year, excluding those with treatment cure, death from disease and self-cure (amounts to 39.7% of disease because 47% are treatment cure, 6.62% die, and 6.62% self-cure). 
...
##########################
  ...
  #model subsequent years based on infections/disease progression in previous year 
    ...
    #11. population at risk at beginning of year (before replacements): sum of all infected and uninfected in previous year 
    results1[i,2] = sum(results1[i-1,3:10]) 
  
  #INFECTION#
      
    #12. new infections: apply ARI to uninfected and current infections + all treatment cures from previous year
    results1[i,3] = input$ari*results1[i-1,10] + input$ari*sum(results1[i-1,3:9]*(1-dis.prob)) + results1[i-1,13]
    
    #13. infection duration 1-5 years (deducting those who progressed to disease or were reinfected)
    ...
    #14. old infections is sum of old infections in previous year and those who advanced from 5yr infection 
    results1[i,9] <- results1[i-1,8]*(1-0.005)*(1-input$ari) + results1[i-1,9]
    
    #15. subtract infected deaths from previous year from infected population - proportional to group size 
    inf.sum = sum(results1[i,3:9])
    results1[i,3] = results1[i,3] - (results1[i,3]/inf.sum)*results1[i-1,16]
    results1[i,4] = results1[i,4] - (results1[i,4]/inf.sum)*results1[i-1,16]
    results1[i,5] = results1[i,5] - (results1[i,5]/inf.sum)*results1[i-1,16]
    results1[i,6] = results1[i,6] - (results1[i,6]/inf.sum)*results1[i-1,16]
    results1[i,7] = results1[i,7] - (results1[i,7]/inf.sum)*results1[i-1,16]
    results1[i,8] = results1[i,8] - (results1[i,8]/inf.sum)*results1[i-1,16]
    results1[i,9] = results1[i,9] - (results1[i,9]/inf.sum)*results1[i-1,16]
    
    #16. self-cures from year prior get added back into model as new infections 
    results1[i,3] = results1[i,3] + results1[i-1,15]
    
    #calculate expected number of infected/uninfected deaths for current year  
      
      #17. infected deaths - expected number of deaths*lifetime risk of infection 
  ...
  #REMAINING UNINFECTED#
    ...
    #22. uninfected: uninfected plus infected/uninfected mortality and death from prevalent disease in previous year
      ...
      #old -> population at risk minus those infected (already accounts for infected/uninfected mortality) and those with disease plus death from prevalent disease
      #results1[i,10] = results1[i,2] - sum(results1[i,3:9]) - results1[i,12] + results1[i,14]
